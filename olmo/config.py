...
class OptimizerConfig(BaseConfig):
    ...
    weight_decay: float = 0 # 0.01
    beta2: float = 0.95
    betas: Tuple[float, float] = (0.95, 0.95)

    use_ewa: bool = True
    multiple_ewa: bool = False
    beta1_decay_ratio: float = -1.0
    beta1_min: float = 0.85
    hardcode_opt_params: bool = False
    ...
    def __post_init__(self):
        ...
        # NOTE only for sweeping multiple EMA values
        # if isinstance(self.ewa_decay, float):
        #     self.ewa_decay = [self.ewa_decay]
        # elif isinstance(self.ewa_decay, str):
        #     self.ewa_decay = json.loads(self.ewa_decay)
...
class SchedulerConfig(BaseConfig):
    ...
    alpha_f: float = 0.1
    # NOTE only for the decay phase of wsd
    decay_ratio: float = 0.1
    t_stable: Optional[Union[int, float]] = None
...
class TrainConfig(BaseConfig):
    ...
    goal_val_loss: Optional[float] = 5.0

    sweep_bs_steps: Optional[str] = None # for sweeping experiments
    sweep_opt_momentum: Optional[str] = "lionw_-1" # for sweeping experiments
    sweep_lr: float = -1 # for sweeping experiments
    sweep: bool = False # for sweeping experiments

    chinchilla_token: int = -1
    ...
    save_interval_unsharded: Optional[List[int]] = None
    """
    ...
    ```bash
    --load_path='${path.last_checkpoint:s3://${CHECKPOINT_BUCKET}/${CHECKPOINT_PATH}}'
    ```
    """
