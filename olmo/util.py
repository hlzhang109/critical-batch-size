...
def get_eval_intervals(cfg):
    eval_intervals = [2**i for i in range(0, 30) if 2**i <= cfg.max_duration] + [500*i for i in range(1, 100000) if 500*i <= cfg.max_duration] + [10**i for i in range(0, 20) if 10**i <= cfg.max_duration]
    if cfg.global_train_batch_size > 256: # enable more frequent eval for large batch size runs
        eval_intervals += [500 * i for i in range(1, 100) if 500 * i <= cfg.max_duration and 500 * i > int(.6 * cfg.max_duration)]
        eval_intervals += [int(n * cfg.max_duration) for n in torch.linspace(0.7, 1, 31)]
    eval_intervals += [cfg.max_duration]
    if cfg.save_num_unsharded_checkpoints_to_keep > 1:
        cfg.save_interval_unsharded = [int(cfg.max_duration/(1.414)**i) for i in range(cfg.save_num_unsharded_checkpoints_to_keep)]
    if cfg.max_duration < 8600:
         eval_intervals += [100*i for i in range(1, 50) if 100*i <= cfg.max_duration]
    elif cfg.max_duration < 12500:
        eval_intervals += [200*i for i in range(1, 40) if 200*i <= cfg.max_duration]
        eval_intervals += [500*i for i in range(1, 21) if 500*i <= cfg.max_duration]
    if cfg.model.max_sequence_length > 512:
        eval_intervals += [100*i for i in range(1, 10000) if 100*i <= cfg.max_duration]
    if 'schedule_free' in cfg.scheduler.name:
        eval_intervals = [100*i for i in range(1, 10000) if 100*i <= cfg.max_duration]
    if cfg.global_train_batch_size > 2**13:
        eval_intervals = [100*i for i in range(1, 10000) if 100*i <= cfg.max_duration]
...
def _s3_find_latest_checkpoint(scheme: str, bucket_name: str, prefix: str) -> Optional[str]:
    ...
    for item in response["CommonPrefixes"]:
        ...
        if step > latest_step or (step == latest_step and not checkpoint_name.endswith("-unsharded")):
            latest_step = step
            latest_checkpoint = f"{scheme}://b/{prefix}"
    return latest_checkpoint
