...
def cross_entropy_loss(
    ...
    lse_square_scale = 0 # 1e-4
    z_loss = lse_square_scale * z_squared
...
class Trainer:
        ...
        if self.cfg.fused_loss:
            ...
            def fused_loss_fn(
                ...
                loss, z_loss = cross_entropy_loss(
                    ...
                    label_smoothing=0.0,
                    logit_scale=1.0,
                    lse_square_scale=0.0, # the parameters for z loss
    ...
    def model_forward(
        self, model, batch: Dict[str, Any], loss_reduction: str = "mean", compute_z_loss: bool = False
    ...
    def eval(self) -> Dict[str, Any]:
        ...
        if self.scheduler is None or self.cfg.optimizer.use_ewa:
            self.optim.eval()
        ...
        for evaluator in self.evaluators:
            ...
            if not self.cfg.optimizer.multiple_ewa:
                eval_metrics.update(metrics)
            else:
                ewa_results = {}
                for metric in metrics:
                    if 'eval' in metric:
                        ewa_results[metric+f"/ewa{self.optim.w_avg_tau}"] = metrics[metric]
                eval_metrics.update(ewa_results)
    ...
    def fit(self):
        ...
        if self.cfg.load_path is not None and self.global_step > 0 and self.cfg.eval_on_load:
            for ewa_tau_id in range(len(self.optim.w_avg_tau_ls)):
        ...
        with torch_profiler as p:
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in self.train_loader:
                    ...
                    if not cancel_initiated and self.global_step in self.eval_intervals:
                    ...
                    if (
                        ...
                        and self.global_step in self.cfg.save_interval_unsharded # == 0
                    ...
                    if not cancel_initiated and self.global_step in self.eval_intervals:
                        ...
                        # Early stop once eval loss is below goal.
                        # if eval_metrics.get("eval/CrossEntropyLoss") is not None:
                        #     cur_val_loss = eval_metrics["eval/CrossEntropyLoss"]
                        #     if cur_val_loss < self.cfg.goal_val_loss:
                        #         log.info(f"Early stopping at step {self.global_step}, \
                        #                    current val loss = {cur_val_loss} < goal validation loss = {self.cfg.goal_val_loss}")
                        #         canceled = True
                    ...
                    # update/anneal beta1:
                    if self.cfg.optimizer.beta1_decay_ratio > 0 and self.global_step > self.cfg.optimizer.beta1_decay_ratio * self.max_steps:
                        new_beta1 = self.cfg.optimizer.momentum - (self.optimizer.momentum - self.optimizer.beta1_min) * ((self.global_step - self.cfg.optimizer.beta1_decay_ratio * self.max_steps) / (self.max_steps - self.cfg.optimizer.beta1_decay_ratio * self.max_steps))
                        # self.cfg.optimizer.momentum - (self.cfg.optimizer.adaptive_beta1 
                        # self.optim.param_groups[0]["betas"] = (new_beta1, self.optim.param_groups[0]["betas"][1])
                        for group in self.optim.param_groups:
